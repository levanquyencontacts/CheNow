...
export function getDateWindow(range: string) {
  ...
  if (range === 'week') {
    const startDate = startOfWeek(todayStart);
  ...
  if (range === 'month') {
    ...
    return {
      days: getDaysBetween(startDate, tomorrowStart),
      endDate: tomorrowStart,
      startDate,
    };
  }
  ...
  if (range === 'year') {
    const startDate = new Date(todayStart.getFullYear(), 0, 1);
