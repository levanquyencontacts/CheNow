...
export class ProductStocksService {
    ...
    constructor(@InjectRepository(ProductStocks) private readonly productStocksRepository: Repository<ProductStocks>) { }
    async findAll() {
        return this.productStocksRepository.find()
    }
    async create(productStocksDto: ProductStocksDto) {
        const newProductStock = this.productStocksRepository.create(productStocksDto)
    ...
    async getProductStockById(id: number) {
        const productStock = await this.productStocksRepository.findOne({ where: { id } })
        if (!productStock) {
